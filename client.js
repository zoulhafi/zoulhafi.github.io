const 	connection = new WebSocket('wss://api.oulhafiane.me:443');
  ...
  yourConnection.onnegotiationneeded = () => {
  	...
  	yourConnection.createOffer().then(offer => {
    	...
    	return yourConnection.setLocalDescription(offer);
  		...
  		send({ type: "offer", offer: yourConnection.localDescription, otherName: connectedUser});
  	})
  };
		  ...
		  navigator.mediaDevices.getUserMedia({ video: true, audio: false }).then((stream) => {
		    ...
		    yourConnection.createAnswer().then( answer => {
		    	...
		    	yourConnection.setLocalDescription(answer).then( () => {
		    		...
		    		send({ type: "answer", answer: yourConnection.localDescription });
		    	});
		    });
		  }).catch( (error) => {
		    console.log(error);
...
connection.onopen = () => { console.log("Connecting..."); };
